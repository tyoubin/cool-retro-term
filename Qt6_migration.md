...
**Strategic Decision:** Instead of porting the old fork, I want to switch to the active, upstream **LXQt `qtermwidget` (master branch)** to ensure future maintainability. I am willing to sacrifice non-core features (like baud rate simulation or specific font tweaks) temporarily to achieve a stable build.
...
2.  **Add the New Submodule:**
    *   Add the upstream repository `https://github.com/lxqt/qtermwidget.git` as a new submodule at the path `qtermwidget`.
...
3.  **Update Build Configuration:**
    ...
    *   *Note:* The file names and folder structure in the LXQt version will differ from the old fork. Do not try to compile yet, just ensure the build system points to the new paths (e.g., look for `.pro` files or `CMakeLists.txt` inside the new directory).
...
2.  Compile the upstream LXQt `qtermwidget` as a library.
3.  Create a minimal "Hello World" QML application that instantiates the upstream `qtermwidget`.
...
1.  Do not modify the upstream `qtermwidget` source code directly.
